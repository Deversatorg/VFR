...
export default function AppLayout({ children }: AppLayoutProps) {
    ...
    const isFullScreenLayout = isStudio || isWardrobe;

    return (
        <div className="min-h-screen bg-[#050505] text-white font-sans flex flex-col pt-20">
            <Navbar />

            {/* If we are in the 3D Studio or Wardrobe, we want a full screen workspace without scroll. Otherwise normal flow */}
            <main className={`flex-1 relative ${isFullScreenLayout ? 'overflow-hidden flex bg-black' : 'max-w-7xl mx-auto w-full p-6 sm:p-8'}`}>
                {children}
            </main>
