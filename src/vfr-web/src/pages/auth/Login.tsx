...
export default function Login() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            try {
                ...
                const adminToken = adminData?.data?.token?.accessToken || adminData?.token?.accessToken || adminData?.accessToken || adminData?.token;
    ...
    const handleGoogleLogin = async () => {
        ...
        setError('');
        try {
            // Mock token retrieval for now, prepare API structure
            const response = await SessionApi.loginGoogle({ idToken: 'mock_google_token' });
    ...
    const handleAppleLogin = async () => {
        ...
        try {
            // Mock token retrieval for now, prepare API structure
            const response = await SessionApi.loginApple({ identityToken: 'mock_apple_token' });
            const token = response.data?.data?.token?.accessToken;
            if (token) {
                login(token);
                navigate('/setup');
            ...
            <div className="flex-1 flex flex-col justify-center items-center p-6 sm:p-12 relative z-10">
                ...
                >
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="space-y-1.5">
                            <div className="flex justify-between items-center ml-1">
                                ...
                                <button type="button" onClick={() => navigate('/forgot-password')} className="text-[12px] font-medium text-primary hover:text-white transition-colors duration-200">Forgot?</button>
                            </div>
                            <input
                                type="password"
                                value={password}
                    ...
                    <div className="mt-6 grid grid-cols-2 gap-4">
                        <button type="button" onClick={handleGoogleLogin} className="flex items-center justify-center gap-3 py-3.5 px-4 bg-[#111111] hover:bg-[#1a1a1a] border border-white/[0.06] rounded-2xl transition-all duration-200 text-sm font-medium text-gray-300 hover:text-white">
                        ...
                        <button type="button" onClick={handleAppleLogin} className="flex items-center justify-center gap-3 py-3.5 px-4 bg-[#111111] hover:bg-[#1a1a1a] border border-white/[0.06] rounded-2xl transition-all duration-200 text-sm font-medium text-gray-300 hover:text-white">
